...
def result_page():
    ...
    # 동물 이모지 추가
    st.subheader(f"당신의 번아웃 수준은 '{stage_info['name']}' {stage_info['emoji']} 단계입니다.")
    st.markdown(f"**총 번아웃 점수: {total_score}점 (최대 132점)**")

    # 번아웃 진행도 시각화 (간단한 프로그레스 바)
    max_total_score = 132
    progress_percentage = (total_score / max_total_score) * 100
    st.progress(int(progress_percentage))
    st.caption(f"번아웃 진행도: {progress_percentage:.1f}%")
